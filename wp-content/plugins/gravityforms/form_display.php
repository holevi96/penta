...
class GFFormDisplay {
	...
	public static function get_form( $form_id, $display_title = true, $display_description = true, $force_display = false, $field_values = null, $ajax = false, $tabindex = 1 ) {
		...
		if ( empty( $confirmation_message ) ) {
			...
			if ( $has_pages ) {
				...
				$previous_button = self::get_form_button( $form['id'], "gform_previous_button_{$form['id']}", $form['lastPageButton'], __( 'Previous', 'gravityforms' ), 'gform_previous_button p-button orange medium', $previous_button_alt, self::get_current_page( $form_id ) - 1 );
	...
	public static function gform_footer( $form, $class, $ajax, $field_values, $previous_button, $display_title, $display_description, $tabindex = 1 ) {
		...
		$button_input = self::get_form_button( $form['id'], "gform_submit_button_{$form['id']}", $form['button'], __( 'Submit', 'gravityforms' ), 'gform_button p-button orange medium', __( 'Submit', 'gravityforms' ), 0 );
	...
	public static function get_field( $field, $value = '', $force_frontend_label = false, $form = null, $field_values = null ) {
		...
		if ( $field->type == 'page' ) {
			if ( $is_entry_detail ) {
				...
				$next_button     = self::get_form_button( $form['id'], "gform_next_button_{$form['id']}_{$field->id}", $field->nextButton, __( 'Next', 'gravityforms' ), 'gform_next_button p-button orange medium', $next_button_alt, $field->pageNumber );
				...
				$previous_button = $field->pageNumber == 2 ? '' : self::get_form_button( $form['id'], "gform_previous_button_{$form['id']}_{$field->id}", $field->previousButton, __( 'Previous', 'gravityforms' ), 'gform_previous_button p-button orange medium', $previous_button_alt, $field->pageNumber - 2 );
