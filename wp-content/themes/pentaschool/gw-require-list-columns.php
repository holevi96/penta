				...
				if($field['id'] == 11){
					...
					foreach($pieces as $piece){
						foreach($this->required_cols as $required_col) {
							...
							if($piece[$required_col] == '') {
								$new_validation_error = true;
								$field['failed_validation'] = true;
								$field['validation_message'] = $field['errorMessage'] ? $field['errorMessage'] : 'A név és az email cím kitöltése kötelező!';
							}
						}						
					}
